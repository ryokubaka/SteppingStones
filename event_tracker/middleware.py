            ...
            with connections['active_op_db'].cursor() as cursor:
                ...
                if not tables_exist:
                    ...
                    if 'django_migrations' not in existing_tables:
                        ...
                        # Fast check: verify the latest migration (0091) is applied
                        # This is much faster than running showmigrations
                        cursor.execute("""
                            SELECT COUNT(*) FROM django_migrations 
                            WHERE app = 'event_tracker' AND name = '0091_add_operation_id_to_existing_credential_tables'
                        """)
                        ...
                        if latest_migration_applied:
                            ...
                            # But also check if this is a very old database that might need all migrations
                            cursor.execute("SELECT COUNT(*) FROM django_migrations WHERE app = 'event_tracker'")
                            applied_count = cursor.fetchone()[0]
                            # If we have some migrations but not the latest, we need to run them
                            has_unapplied = True
